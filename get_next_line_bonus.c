...
/*   Updated: 2022/03/07 18:21:37 by amaarouf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 0)
		return (NULL);
	text = check_remainder(&remainder[fd], fd);
	if (text)
	{
		if (remainder[fd] == NULL)
			remainder[fd] = ft_strdup("");
		line = ft_strjoin_free(remainder[fd], get_line(text, &line_len), 1);
		remainder[fd] = NULL;
		if (line != NULL && text[line_len])
			remainder[fd] = ft_strdup(text + line_len + 1);
